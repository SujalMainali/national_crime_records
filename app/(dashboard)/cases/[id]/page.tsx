      ...
      <div className="animate-fade-in">
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            {/* Key Details Grid */}
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-lg">
              <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-4">Incident Details</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  <p className="text-xs text-slate-400 uppercase font-semibold">Location</p>
                  <p className="text-[#1e3a5f] font-medium mt-1 text-sm">{caseData.incident_location || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400 uppercase font-semibold">District</p>
                  <p className="text-[#1e3a5f] font-medium mt-1 text-sm">{caseData.incident_district || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400 uppercase font-semibold">Date & Time</p>
                  <p className="text-[#1e3a5f] font-medium mt-1 text-sm">{caseData.incident_date_time ? new Date(caseData.incident_date_time).toLocaleString() : 'N/A'}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400 uppercase font-semibold">Section of Law</p>
                  <p className="text-[#1e3a5f] font-medium mt-1 text-sm">{caseData.crime_section || 'N/A'}</p>
                </div>
              </div>
            </div>
            ...
            {/* Assigned Officer */}
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-lg">
              <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-4">Assigned Officer</h3>
              {caseData.officer_name ? (
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#0c2340] to-[#1e3a5f] flex items-center justify-center text-white font-bold shadow">
                      {caseData.officer_name?.split(' ').map((n: string) => n[0]).join('').substring(0, 2).toUpperCase()}
                    </div>
                    <div>
                      <p className="text-[#0c2340] font-semibold">{caseData.officer_name}</p>
                      <p className="text-slate-500 text-sm">{caseData.officer_rank || 'Officer'}{caseData.officer_badge ? ` · Badge: ${caseData.officer_badge}` : ''}</p>
                    </div>
                  </div>
                  {caseData.officer_id && (
                    <Link
                      href={`/officers/${caseData.officer_id}`}
                      className="px-4 py-2 text-sm text-[#0c2340] hover:bg-[#0c2340]/5 rounded-lg transition-all font-medium"
                    >
                      View Profile →
                    </Link>
                  )}
